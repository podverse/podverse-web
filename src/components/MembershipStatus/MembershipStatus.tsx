            ...
            <div className='membership-status-wrapper'>
              <h2>{t('Status')}</h2>
              {
                ...
                && isBeforeDate(userInfo?.membershipExpiration) && (
                  <>
                    ...
                    <p>{t('Ends')}{new Date(userInfo?.membershipExpiration).toLocaleString()}</p>
                  </>
                )
              }
              {
                ...
                && isBeforeDate(userInfo?.freeTrialExpiration) && (
                  <>
                    ...
                    <p>{t('Ends')}{new Date(userInfo?.freeTrialExpiration).toLocaleString()}</p>
                  </>
                )
              }
              {
                ...
                && !isBeforeDate(userInfo?.freeTrialExpiration) && (
                  <>
                    ...
                    <p>{t('Ended')}{new Date(userInfo?.freeTrialExpiration).toLocaleString()}</p>
                    <p>{t('TrialEnded')}</p>
                  </>
                )
              }
              {
                ...
                && !isBeforeDate(userInfo?.membershipExpiration) && (
                  <>
                    <div className='membership-status is-expired'>{t('Expired')}</div>
                    <p>{t('Ended')}{new Date(userInfo?.membershipExpiration).toLocaleString()}</p>
